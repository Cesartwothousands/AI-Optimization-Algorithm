...
def _graph_search(graph, start_node_id, goal_node_id, _add_to_queue, is_queue_empty, _pop, _heuristic, a=1, b=1):
    ...
    _add_to_queue(start_node_id, 0, 0 +
                  _heuristic(graph, start_node_id) * b, True)
    ...
    while not is_queue_empty():
        ...
        if node_id != start_node_id:
            node[3] = graph[parent_node_id][3] + \
                graph[parent_node_id][6][node_id]
        ...
        for n in range(0, len(node[5])):
            nbr = node[5][n]
            if not graph[nbr[0]][1]:
                _add_to_queue(nbr[0], node_id, graph[node_id][6]
                              [nbr[0]] * a + _heuristic(graph, nbr[0]) * b)
